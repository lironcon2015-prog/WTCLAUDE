...
ולא בקשה חד-פעמית. סייגים שגוברים עליו:

- **`CLAUDE.md` גובר** על כל כלל כאן.
- **שינוי קוד אפליקציה → קידום `CACHE_VERSION` ב-`sw.js`**, אחרת ה-PWA ממשיכה
  להגיש את הגרסה הישנה.
- **כישלון מדווח.** בדיקה שנפלה או שלב שדולג נאמרים במפורש.
- **סיכום קצר בעברית בסוף כל משימה.** "Silent completion" חל על שלבי ביניים.
